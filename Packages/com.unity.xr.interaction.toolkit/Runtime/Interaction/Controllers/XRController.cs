...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public class XRController : MonoBehaviour
    {
        ...
        void UpdateInput()
        {
            ...
            HandleInteractionAction(controllerNode, s_InputAxisNames[(int)m_SelectUsage], ref m_SelectInteractionState, InteractionTypes.select);
            HandleInteractionAction(controllerNode, s_InputAxisNames[(int)m_ActivateUsage], ref m_ActivateInteractionState, InteractionTypes.activate);
            HandleInteractionAction(controllerNode, s_InputAxisNames[(int)m_UIPressUsage], ref m_UIPressInteractionState, InteractionTypes.uiPress);

            UpdateControllerModelAnimation();
        }

        bool GetSimulator(InteractionTypes interactionStateType)
        {
            ...
            switch (interactionStateType)
            {
                ...
                case InteractionTypes.select:
                    return ((controllerNode == XRNode.LeftHand && Input.GetAxis("LeftGrab") >= m_AxisToPressThreshold) || (controllerNode == XRNode.RightHand && Input.GetAxis("RightGrab") >= m_AxisToPressThreshold));
        ...
        void HandleInteractionAction(XRNode node, string usage, ref InteractionState interactionState, InteractionTypes interactionStateType)
        {
            float value = 0.0f;
            if ((inputDevice.isValid && inputDevice.TryGetFeatureValue(new InputFeatureUsage<float>(usage), out value) &&
                value >= m_AxisToPressThreshold) || GetSimulator(interactionStateType))
            {
