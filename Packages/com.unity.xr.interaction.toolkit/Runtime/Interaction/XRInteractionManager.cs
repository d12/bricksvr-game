...
namespace UnityEngine.XR.Interaction.Toolkit
{
	...
	{        
        ...
        private List<XRBaseInteractable> sortedTargets = new List<XRBaseInteractable>();
        ...
        public void ForceHoverExit(XRBaseInteractor interactor, XRBaseInteractable interactable)
        {
	        interactor.OnHoverExit(interactable);
	        interactable.OnHoverExit(interactor);
        }

        public void ForceDrop(XRBaseInteractor interactor, XRBaseInteractable interactable)
        {
	        interactor.OnSelectExit(interactable);
	        interactable.OnSelectExit(interactor);
        }

        public void ForceSwap(XRBaseInteractor oldInteractor, XRBaseInteractor newInteractor,
	        XRBaseInteractable interactable)
        {
	        oldInteractor.OnSelectExit(interactable);
	        interactable.OnSelectExit(oldInteractor);

	        newInteractor.OnSelectEnter(interactable);
	        interactable.OnSelectEnter(newInteractor);
        }
		...
		public void ClearInteractorHover(XRBaseInteractor interactor, List<XRBaseInteractable> validTargets)
		{
        ...
        void InteractorSelectValidTargets(XRBaseInteractor interactor, List<XRBaseInteractable> validTargets)
        {
	        if (interactor.isSelectActive)
	        {
		        ...
		        sortedTargets.Clear();
		        sortedTargets.AddRange(validTargets);
		        Vector3 interactorPos = interactor.transform.position;

		        sortedTargets.Sort((a, b) => (a.transform.position - interactorPos).sqrMagnitude > (b.transform.position - interactorPos).sqrMagnitude ? 1 : -1);
		        for (var i=0; i < sortedTargets.Count && interactor.isSelectActive; ++i)
		        {
			        if (interactor.CanSelect(sortedTargets[i]) && sortedTargets[i].IsSelectableBy(interactor) &&
			            interactor.selectTarget != sortedTargets[i])
			        {
				        SelectEnter(interactor, sortedTargets[i]);
			        }
		        }
	        }
        }
